inventoryApp.controller("salesReportController", function ($scope, $location, CRUDOperations) {
	...
	$scope.fetchReportData = function(){
		if ($scope.selectedDateRange) {
			...
			var warehouse = "";
			var promiseget = CRUDOperations.get("/api/kits/getSalesReportByDateRange?fromDate="+$scope.fromDate+"&toDate="+$scope.toDate + (($scope.selectedWarehouse)?"&warehouseId="+$scope.selectedWarehouse:"&warehouseId="));
